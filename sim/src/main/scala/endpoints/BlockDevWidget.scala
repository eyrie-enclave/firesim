...
class BlockDevWidget(implicit p: Parameters) extends EndpointWidget()(p) {
  ...
  chisel3.experimental.withReset(reset.toBool || targetReset) {
    ...
    wAckBuf.io.deq.ready := tFireHelper.fire(wAckStallN) && target.resp.ready && returnWrite
    rRespBuf.io.deq.ready := tFireHelper.fire(rRespStallN) && target.resp.ready && readRespBusy
